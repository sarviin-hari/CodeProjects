...
if opt == 1:
    ...
    for vehc in vehicle_fleet:
        ...
        print("~~> Path: " + str(trip))
        print("~~> Vehc: " + str(vehc))
        # if time is math.inf, print math.inf, else print the corresponding time taken
        if path_finding.valid_time_list[i - 1] == math.inf:
            print("~~> Time: " + str(math.inf))
        else:
            print("~~> Time: " + str(sum(path_finding.valid_time_list[i - 1])))  # 1356807575
        i += 1
        print("")

    number = shortest_path_trip(len(vehicle_fleet))  # calls the shortest_path_trip function to get the user choice short path
    best_trip = trip_list[number - 1]  # gets the corresponding best trip of the trip based on user choice

    # if no trip available then print error message
    if best_trip == None:
        print("The travel is not possible... Re-run the program to try again")
    else:
        best_vehicle = vehicle_fleet[number - 1]  # gets the corresponding vehicle of the trip based on user choice
        cities = trip_list[number - 1].city_list  # gets the corresponding cities of the trip based on user choice
        time_list = path_finding.valid_time_list[number - 1]  # gets the corresponding time list of the trip based on user choice
        best_trip_time = sum(time_list)  # best trip time is calculated by finding the sum of timelist
        progress_bar(best_trip, best_vehicle, cities, time_list, best_trip_time)  # call progress bar function to output the progress bar
