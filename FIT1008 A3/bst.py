...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        ...
        while check is not None:
            # print(2, check.right, check.right.key)
            if check.key > current.key:
    ...
    # def get_successor_aux(self, current, value):
    ...
    def kth_smallest(self, k: int, current: TreeNode) -> TreeNode:
        ...
        val = self.inorder_aux(current, k)
        self.counter = 0
        return val
    ...
    def inorder_aux(self, current: T, ini_k, boolean=True):
        """
        ...
        if current is not None:  # if not a base case
            ...
            val = self.inorder_aux(current.left, ini_k, boolean)
            print(type(val))

            if not isinstance(val, TreeNode):
                self.counter += 1
                print(self.counter, current)
            ...
            val = self.inorder_aux(current.right, ini_k, boolean)
