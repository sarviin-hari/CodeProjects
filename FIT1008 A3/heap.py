...
class MaxHeap(Generic[T]):
    ...
    def heapify(self, replacement_arr) -> None:
        """
        Apply bottom-up heap construction in O(n) time

        Args:
        - replacement_arr: List of Beehive instance
        ...
        Complexity:
        - Worst case: O(n), where n is the length of the replacement_arr
        - Best case: O(n), where n is the length of the replacement_arr
        - Reason: The first loop takes O(n) complexity to restore all the previous
                  elements to new elements from the replacement_arr, and has the complexity
                  of O(n//2) which is also of O(n) complexity
        """

        for i in range(self.length):
            self.the_array[i + 1] = replacement_arr[i]
